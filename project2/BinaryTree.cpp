...
template<class T>
void BinaryTree<T>::empty( BTNode* node ){
  if(!node){
    return;
  }
  empty(node->left);
...
template<class T>
bool BinaryTree<T>::equal( const BTNode<T>* node1, const BTNode<T>* node2 ) const
{
  if (!node1 && !node2)
    return true;
  else if ((!node1 && node2) || (node1 && !node2)) // xor
    return false;
  else if(node1->elem == node2->elem && //preorder traversal
...
template<class T>
int BinaryTree<T>::height( const BTNode<T>* node ) const 
{
  if (!node)
    return 0;
...
template<class T>
int BinaryTree<T>::leaf_count( const BTNode<T>* node ) const
{
  if (node->is_leaf())
    return 1;
